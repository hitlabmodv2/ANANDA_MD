...
END:VCARD
        ...
        // ===== BOT RESPONSE LOGGER =====
        // Log every outgoing message (bot replies). Incoming user messages are NOT logged here.
        const _origSend = sock.sendMessage.bind(sock);
        sock.sendMessage = async function (jid, content, options) {
                try {
                        const target = String(jid || '');
                        const isChat = /@(s\.whatsapp\.net|g\.us|lid)$/.test(target) && target !== 'status@broadcast';
                        if (isChat && content && typeof content === 'object') {
                                let kind = 'unknown';
                                let preview = '';
                                if (content.text != null)            { kind = 'text';     preview = String(content.text); }
                                else if (content.caption != null && (content.image || content.video || content.document)) {
                                        kind = content.image ? 'image' : content.video ? 'video' : 'document';
                                        preview = String(content.caption);
                                }
                                else if (content.image)              kind = 'image';
                                else if (content.video)              kind = 'video';
                                else if (content.audio)              kind = content.ptt ? 'voice' : 'audio';
                                else if (content.sticker)            kind = 'sticker';
                                else if (content.document)           kind = 'document';
                                else if (content.contacts)           kind = 'contact';
                                else if (content.location)           kind = 'location';
                                else if (content.react)              { return _origSend(jid, content, options); } // skip log: react bukan respon utama
                                else if (content.buttons)            { kind = 'buttons'; preview = String(content.text || content.caption || ''); }
                                else if (content.templateButtons)    { kind = 'template'; preview = String(content.text || content.caption || ''); }
                                else if (content.interactiveMessage) { kind = 'interactive'; }
                                else if (content.delete)             kind = 'delete';
                                else if (content.edit)               { kind = 'edit'; preview = String(content.text || ''); }
                                else if (content.forward)            kind = 'forward';
                                else if (content.poll)               { kind = 'poll'; preview = String(content.poll.name || ''); }

                                if (kind !== 'react' || preview) {
                                        const t  = new Date().toLocaleTimeString('id-ID', { hour12: false });
                                        const to = target.split('@')[0];
                                        const grp = target.endsWith('@g.us') ? '[GRUP] ' : '';
                                        const snippet = preview.replace(/\s+/g, ' ').trim().slice(0, 80);
                                        const tail = snippet ? ` "${snippet}${preview.length > 80 ? '…' : ''}"` : '';
                                        console.log(`\x1b[36m[BOT ${t}]\x1b[0m → ${grp}${to} \x1b[33m(${kind})\x1b[0m${tail}`);
                                }
                        }
                } catch (_) { /* never break sending because of logging */ }
                                ...
                                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                                        ...
                                        download: {
                                                value(saveToFile = false) {
                                                        ...
                                                        if (!mtype) return Promise.resolve(Buffer.alloc(0));
                                                        const mediaMsg = this.mediaMessage;
                                                        if (!mediaMsg || !mediaMsg[mtype]) return Promise.resolve(Buffer.alloc(0));
                                                        return self.conn?.downloadM(mediaMsg[mtype], mtype.replace(/message/i, ''), saveToFile);
                                                },
                                                enumerable: true,
                                                configurable: true,
                                        },
