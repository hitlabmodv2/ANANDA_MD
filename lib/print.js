...
export default async function (m, conn = { user: {} }) {
        const senderNumber = PhoneNumber('+' + m.sender.split('@')[0]).getNumber('international');
        const _name = conn.getName(m.sender);
        const sender = senderNumber + (_name && _name !== senderNumber ? ' ~' + _name : '');
        const chatNumber = m.chat.endsWith('@s.whatsapp.net') ? PhoneNumber('+' + m.chat.split('@')[0]).getNumber('international') : '';
        const _chat = conn.getName(m.chat);
        const chat = _chat && _chat !== chatNumber ? _chat : '';
