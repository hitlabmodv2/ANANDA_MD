...
import nsfwchecker from './lib/nsfwchecker.js';
import { honoluluSpeak } from './lib/honolulu-speak.js';
        ...
        try {
                ...
                for (let name in global.plugins) {
                        ...
                        ////if (name != 'tools-delete.js' && chat?.isBanned) return; // Except this
                                ...
                                try {
                                        const _t = new Date().toLocaleTimeString('id-ID', { hour12: false });
                                        const _from = String(m.sender || '').split('@')[0];
                                        const _name = (typeof conn.getName === 'function' ? conn.getName(m.sender) : '') || '-';
                                        const _grp = m.isGroup ? `[GRUP ${String(m.chat).split('@')[0]}] ` : '';
                                        const _argsTxt = (args && args.length) ? ' ' + args.join(' ') : '';
                                        const _cmd = `${usedPrefix || ''}${command}${_argsTxt}`.trim().replace(/\s+/g, ' ').slice(0, 100);
                                        console.log(`\x1b[35m[CMD ${_t}]\x1b[0m ${_grp}${_from} (${_name}) → \x1b[32m${_cmd}\x1b[0m \x1b[90m[${name}]\x1b[0m`);
